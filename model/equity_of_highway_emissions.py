...
root = r""
# ^This must be set to where you save the root file directory
...
#------...
#---state shp 2016 simplified
#------NewHampshire.shp
#------...
#---State_median_housing values.csv
#---FIPS names.csv
...
root_acs = root + "\\ACS 2019"
root_road = root + "\\state shp 2016 simplified"
root_dmg = root + "\\state shp air dmg"
root_results = root + "\\results"

### Read in some of the files used ###
files = os.listdir(root_acs)

house_value = pandas.read_csv(r"C:\Users\thor.dodson\Desktop\Reading Materials\ECM\Housing Density\State_median_housing values.csv")
county_names = pandas.read_csv(r"C:\Users\thor.dodson\Desktop\Reading Materials\ECM\Housing Density\\FIPS names.csv")
...
concentration_gradient = pandas.read_csv(root+"\\concentrations_ex.csv")
...
STATES_DATA = pandas.read_csv(root+"\\States_Data.txt",header=None,dtype=str).values.tolist()
...
nfha_data = pandas.DataFrame([[.024121,.009285],[.00481,.001992],[.00198,.002241],[.0053,.002213],[.00225,.000592]],columns=cols)
nfha = pool(nfha_data['Beta'],nfha_data['Std'])

#hospital admission, all respiratory
haar_data = pandas.DataFrame([[.00207,.00045],[.0007,.00096],[.002,.00434],[.00332,.00104],[.0022,.00073]],columns=cols)
haar = pool(haar_data['Beta'],haar_data['Std'])

#hospital admission, cardiovascular (except heart attacks)
hac_data = pandas.DataFrame([[.00189,.00028],[.00071,.00013],[.00068,.00021],[.0008,.00011],[.0014,.00034]],columns=cols)
...
#asthma exacerbation
ae_data = pandas.DataFrame([[.000985,.000747],[.002565,.001335],[.001942,.000803],[.01222,.013849]],columns=cols)
...
def est_population(road,x,age_shp):
    ...
    c_[ages[0]] = c_['area']/c_['Area']*(c_['B01001e3']+c_['B01001e4']+c_['B01001e5']+c_['B01001e6']+c_['B01001e27']+c_['B01001e28']+c_['B01001e29']+c_['B01001e30'])
    c_[ages[1]] = c_['area']/c_['Area']*(c_['B01001e7']+c_['B01001e8']+c_['B01001e9']+c_['B01001e10']+c_['B01001e31']+c_['B01001e32']+c_['B01001e33']+c_['B01001e34'])
    ...
    c_[ages[5]] = c_['area']/c_['Area']*(c_['B01001e17']+c_['B01001e18']+c_['B01001e19']+c_['B01001e41']+c_['B01001e42']+c_['B01001e43'])
    c_[ages[6]] = c_['area']/c_['Area']*(c_['B01001e20']+c_['B01001e21']+c_['B01001e22']+c_['B01001e44']+c_['B01001e45']+c_['B01001e46'])
    ...
    c['population'] = c.apply(lambda row: {ages[0]:row[ages[0]],ages[1]:row[ages[1]],ages[2]:row[ages[2]],
                                          ...
                                          ages[6]:row[ages[6]],ages[7]:row[ages[7]],ages[8]:row[ages[8]]},1)
    c = c[['population']]
...
def health_dmg(concentration,population):
    ...
    for d in dist:
        for category in categories:
            for age in ages:
                dmg += concentration[d]*population[age]*dist_width_adj[d]*health[category+age]['impact']*health[category+age]['baseline']/100*health[category+age]['value']
...
def equity(c):
    ...
    hispanic = gpd.read_file(gdb, layer="X03_Hispanic_or_Latino_Origin"); hispanic = hispanic.drop('geometry',1)
    ...
    age = gpd.read_file(gdb, layer="X01_Age_and_Sex"); age = age.drop('geometry',1)
    ...
    #gdb = root+"\\state shp 2016 simplified_0\\"+state+".shp"
    gdb = root_road+"\\"+state+".shp"
    ...
    road['SPEED_LIMI'] = road.apply(lambda row: adj_speed(row['SPEED_LIMI']),1)
    ...
    road['population'] = road['population'].replace(np.nan,None)
    ...
    road['emissions'] = road.apply(lambda row: calc_emissions(row['AADT_PASSENGER'],row['AADT_SINGL'],row['AADT_COMBI'],
                                                             row['SPEED_LIMI'],row['length'],row['width'],row['restricted']),1)
    
    road['concentration'] = road.apply(lambda row: calc_concentrations(row['emissions']),1)
    ...
    road['dmg'] = road.apply(lambda row: health_dmg(row['concentration'],row['population']),1)
    ...
    road_write.to_file(root_dmg+"\\"+state+"_air_dmg.shp")
...
results2.to_csv(root_results+"\\Equity_long_air.csv")
county.to_csv(root_results+"\\Equity_county_air.csv")
census.to_csv(root_results+"\\Equity_census_air.csv")
